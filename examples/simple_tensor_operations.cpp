...
void basic_tensor_operations() {
  ...
  std::cout << "  identity: " << shape(ctx, eye_tensor)[0] << "×" << shape(ctx, eye_tensor)[1]
            << std::endl;
  ...
  std::cout << "  eye[1,1] = " << std::real(eye_elem) << " + " << std::imag(eye_elem) << "i"
            << std::endl;
...
void tensor_contraction_example() {
  ...
  std::cout << "    D(3×4) × E(4×5) = F(" << shape(ctx, F)[0] << "×" << shape(ctx, F)[1] << ")"
            << std::endl;
