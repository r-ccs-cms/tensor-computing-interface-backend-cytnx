...
using storage_decay_t = std::decay_t<Storage>;

template <typename Storage>
inline constexpr bool is_path_storage_v =
    std::is_same_v<storage_decay_t<Storage>, std::filesystem::path> ||
    std::is_convertible_v<storage_decay_t<Storage>, std::string>;

template <typename Storage>
inline constexpr bool is_input_stream_v =
    std::is_base_of_v<std::istream, storage_decay_t<Storage>>;

template <typename Storage>
inline constexpr bool is_output_stream_v =
    std::is_base_of_v<std::ostream, storage_decay_t<Storage>> &&
    !is_input_stream_v<Storage>;

inline std::filesystem::path to_path(const std::filesystem::path &path) {
    return path;
}

inline std::filesystem::path to_path(std::filesystem::path &&path) {
...
inline std::filesystem::path to_path(const char *path) {
    return std::filesystem::path(path);
}

template <typename Storage>
inline std::filesystem::path to_path(Storage &&storage) {
    if constexpr (std::is_convertible_v<Storage, std::string>) {
        return std::filesystem::path(std::string(std::forward<Storage>(storage)));
    } else {
        static_assert(
            dependent_false_v<Storage>,
            "Unable to convert storage to filesystem path"
        );
    }
}
...
inline std::filesystem::path make_temp_cytn_file() {
    auto base = std::filesystem::temp_directory_path() / ("tci-io-" + std::to_string(std::hash<std::thread::id>{}(std::this_thread::get_id())));
...
inline cytnx::Tensor load_from_path(
    context_handle_t<cytnx::Tensor> &ctx,
    const std::filesystem::path &path
) {
    cytnx::Tensor result;
...
> {
    ...
    template <typename StorageLike>
    static void save(
        context_handle_t<cytnx::Tensor> &,
        const cytnx::Tensor &,
        StorageLike &
    ) {
        static_assert(
            dependent_false_v<StorageLike>,
