...
template <typename...>
struct dependent_false_helper : std::false_type {};

template <typename... Args>
inline constexpr bool dependent_false_v = dependent_false_helper<Args...>::value;

template <typename TenT, typename Storage, typename Enable = void>
struct storage_adapter {
    ...
    static void load(
        ...
        static_assert(dependent_false_v<TenT, StorageLike>, "Unsupported storage type for tci::load");
    }

    template <typename StorageLike>
    static TenT load_out(
    ...
    static void save(
        ...
        static_assert(dependent_false_v<TenT, StorageLike>, "Unsupported storage type for tci::save");
    }
};
...
inline void load(
    ...
    detail::storage_adapter<TenT, std::decay_t<Storage>>::load(
        ctx,
        std::forward<Storage>(strg),
        a
    );
}
