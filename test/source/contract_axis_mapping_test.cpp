...
TEST_CASE("Contract Axis Mapping Debug - NCON notation") {
    ...
    SUBCASE("Simple 2D contract test") {
        ...
        auto A = tci::random<Tensor>(context, tci::shape_t<Tensor>{3, 4},
                                   []() { return 1.0; });
        auto B = tci::random<Tensor>(context, tci::shape_t<Tensor>{4, 5},
                                   []() { return 1.0; });
    ...
    SUBCASE("4D time evolution contract test - exact replica") {
        ...
        auto theta = tci::random<Tensor>(context, tci::shape_t<Tensor>{5, 2, 2, 5},
                                       []() { return 1.0; });
        auto u = tci::random<Tensor>(context, tci::shape_t<Tensor>{2, 2, 2, 2},
                                   []() { return 1.0; });
        ...
        std::cout << "\nContract: theta{0, -1, -2, 3} × u{-1, -2, 1, 2} = result{0, 1, 2, 3}" << std::endl;
        std::cout << "Expected: contract theta[1,2] with u[0,1], result should be (5,2,2,5)" << std::endl;
        ...
        bool correct_shape = (result_shape.size() == 4 &&
                            result_shape[0] == 5 && result_shape[1] == 2 &&
    ...
    SUBCASE("Step-by-step axis analysis") {
        ...
        auto A = tci::random<Tensor>(context, tci::shape_t<Tensor>{2, 3},
                                   []() { return 1.0; });
        auto B = tci::random<Tensor>(context, tci::shape_t<Tensor>{3, 4},
                                   []() { return 1.0; });
