...
TEST_CASE("TCI Matrix Decomposition - Truncated SVD") {
  ...
  SUBCASE("Truncated SVD with chi_max constraint") {
    ...
    CHECK_NOTHROW(tci::trunc_svd(ctx, matrix, 1, u, s_diag, v_dag, trunc_err, tci::bond_dim_t<tci::CytnxTensor<cytnx::cytnx_complex128>>(2), 0.1));
...
TEST_CASE("TCI Truncated SVD") {
  ...
  SUBCASE("Truncated SVD with max bond dimension") {
    ...
    CHECK_NOTHROW(tci::trunc_svd(ctx, matrix, 2, u, s_diag, v_dag, trunc_err, tci::bond_dim_t<tci::CytnxTensor<cytnx::cytnx_complex128>>(2), 0.5));
