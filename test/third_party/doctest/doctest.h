    ...
    inline int run_all_tests() {
      ...
      for (const auto& test : registry()) {
        ...
        try {
          ...
          std::cerr << test.file << ":" << test.line << ": uncaught exception in test '"
                    << test.name << "': " << ex.what() << std::endl;
          ++failure_count();
        } catch (...) {
      ...
      std::cout << "[doctest-lite] " << total << " test case(s), " << failure_count()
                << " failure(s)" << std::endl;
...
#define TEST_CASE(name)                                                                   \
  ...
  static ::doctest::detail::Registrar DOCTEST_ANON_REG(DOCTEST_ANON_FUNC, name, __FILE__, \
                                                       __LINE__);                         \
  static void DOCTEST_ANON_FUNC()

#define SUBCASE(name)                                    \
  for (int DOCTEST_CAT(_doctest_subcase_, __LINE__) = 0; \
       DOCTEST_CAT(_doctest_subcase_, __LINE__)++ == 0;)
